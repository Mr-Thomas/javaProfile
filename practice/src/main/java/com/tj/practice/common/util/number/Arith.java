...
public class Arith {
     ...
     * 默认舍入值
     */
    private static final int DEF_DIV_ROUNT = 4;
    ...
    /**
     * 保留小数位四舍五入，小数位不足补0，入参v为空时返回BigDecimal.valueOf(0);
     *
     * @param v     四舍五入的对像 123.699
     * @param cacle 保留小数位 2
     * @return 数值 123.70
     */
    public static BigDecimal round(BigDecimal v, int cacle) {
        if(v == null){
            return BigDecimal.valueOf(0);
        }
        return v.setScale(cacle, BigDecimal.ROUND_HALF_UP);
    }
     ...
     *
     * @param v     舍入对像
     * @param rount 舍入方式0-9 rount舍rount+1入  例：rount=5 则表示五舍六入
     * @param scale 保留精度 正数表示小数点后几位，负数表示小数点前几位，例如：-1表示进位到十位数
     * @return 数值
     */
    public static BigDecimal round(BigDecimal v, int rount, int scale) {
        double d = v.doubleValue();
        if(d == 0){
            return v;
        }
        //与四舍五入对比的差额
        BigDecimal bias = new BigDecimal(rount - DEF_DIV_ROUNT);
        //舍入差额精度
        BigDecimal s = new BigDecimal(Double.toString(Math.pow(DEF_DIV_SCALE, scale + 1)));
        if(d < 0){
            v = v.add(bias.divide(s));
        }else{
            v = v.subtract(bias.divide(s));
        }
        BigDecimal v2 = v.setScale(scale, BigDecimal.ROUND_HALF_UP);
        if (scale <= 0) {//去除科学计数法
            v2 = new BigDecimal(v2.intValue());
        }
        return v2;
    }
     ...
     * @param v     舍入对像
     * @param rount 舍入方式0-9 rount舍rount+1入  例：rount=5 则表示五舍六入
     * @param scale 保留精度 正数表示小数点后几位，负数表示小数点前几位
     * @return 数值
     */
    public static double round(double v, int rount, int scale) {
        if(v == 0){
            return v;
        }
        BigDecimal v1 = new BigDecimal(Double.toString(v));
        //与四舍五入对比的差额
        BigDecimal bias = new BigDecimal(rount - DEF_DIV_ROUNT);
        ...
        BigDecimal s = new BigDecimal(Double.toString(Math.pow(DEF_DIV_SCALE, scale + 1)));
        if(v < 0){
            v1 = v1.add(bias.divide(s));
        }else{
            v1 = v1.subtract(bias.divide(s));
        }
        BigDecimal v2 = v1.setScale(scale, BigDecimal.ROUND_HALF_UP);
   ...
   /* *//**
    ...
    public static Double Obj2Double(Object o) {
        ...
        return Arith.round(ddd, DEF_DIV_SCALE_TWO);
    }*/
